...
public class AudioController : SingletonMono<AudioController>
{
    ...
    {
        foreach(var v in GlobalData.Settings.volumes) v.Value.onChange += ((volume)=>SetVolume(v.Key + ',' + volume.ToString()));
    ...
    {
        foreach(var v in GlobalData.Settings.volumes) SetVolume(v.Key + ',' + v.Value.value.ToString());
    }
    ...
    public void SetVolume(string parameter)
    {
        ...
        volume = volume == 0 ? -144 : Mathf.Log10(volume) * 20;
    ...
    public void SetVolumePrefs(string parameter)
    {
        ...
        GlobalData.Settings.volumes[group].value = volume;
    }
}